...
theme(cook()); // pour avoir la dernier theme selectione
 ...
 function cook(){
  if(document.cookie){
    ...
    for(let i = 0 ; i < 3 ; i++){
      console.log(coooke.search(themeTab[i]));
 ...
 function theme(color){
  ...
  let nav = document.getElementById('job');
  let backGround = document.getElementById('wallpaper');
  let ligne = document.getElementsByClassName('ligne');
  


  switch(color){ // change les colors 
      case 'red'      : clr = "#fff0f0"; bgclr = "#ffccc6"; break; 
      case 'blue'     : clr = "#d2e8ff"; bgclr = "#95bcfc"; break; 
      case 'green'    : clr = "#eefae3"; bgclr = "#88e2ae"; break; 
      default  : clr = "#f2fcff"; bgclr = "#adddff"; break; 
  }

  // affecter les nouvelles colors
  nav.style.backgroundColor = clr; 
  bodyleft.style.backgroundColor = clr;
  backGround.style.backgroundColor = bgclr;

  for(let i = 0 ; i<ligne.length ; i++){
      ligne[i].style.backgroundColor = bgclr; 
  }
}
...
function gotoAfficheVille(){
  //Pour aller a "Affiche ville.php"
  return window.location.href = 'affiche ville.php';
}
