...
function OpenSession()
{
    ...
    if (!($connect)) {
        ...
        $sql2 = $connect->query("SELECT * FROM continent;");
...
function rechercher()
{
    ...
    if (!($connexion)) {
        ...
        if ($continant == '' && $pays == '' && $ville == '')
            ...
            $sql = "SELECT * from ville  NATURAL join pays NATURAL join continent Where nomcon like '$continant%' and nompay like '$pays%' and nomvil like '$ville%';";
            $sql = $connexion->query($sql);
